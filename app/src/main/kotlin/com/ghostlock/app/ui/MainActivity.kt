...
import com.ghostlock.app.ai.ui.AiChatScreen
import com.ghostlock.app.ai.ui.AiChatViewModel
import com.ghostlock.app.ai.ui.AiSettingsScreen
import com.ghostlock.app.ai.ui.AiSettingsViewModel

private enum class Screen { MAIN, AI_CHAT, AI_SETTINGS }
...
class MainActivity : ComponentActivity() {
    ...
    private val aiChatViewModel by viewModels<AiChatViewModel> {
        viewModelFactory {
            initializer {
                AiChatViewModel(
                    getApplication(),
                    (application as GhostlockApplication).createRepository(),
                )
            }
        }
    }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            when (screen) {
                Screen.MAIN -> GhostlockRoute(
                    viewModel = viewModel,
                    onEffect = ::handleEffect,
                    onOpenAiChat = { screen = Screen.AI_CHAT },
                )
                Screen.AI_CHAT -> AiChatScreen(
                    viewModel = aiChatViewModel,
                    onBack = { screen = Screen.MAIN },
                    onOpenSettings = { screen = Screen.AI_SETTINGS },
                )
                Screen.AI_SETTINGS -> AiSettingsScreen(
                    viewModel = aiSettingsViewModel,
                    onBack = { screen = Screen.AI_CHAT },
                )
            }
        }
        setupSystemBars()
